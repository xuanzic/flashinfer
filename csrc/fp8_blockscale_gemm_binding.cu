...
class Fp8BlockScaleGemmRunner : public tvm::ffi::ModuleObj {
 ...
 private:
  void runGemm(const TensorView& input, const TensorView& weight, const TensorView& output,
    ...
    TVM_FFI_ICHECK_EQ(weight.size(1), shape_k) << "Weight K dimension must match input K";
    TVM_FFI_ICHECK_EQ(output.size(0), shape_m) << "Output M dimension must match input M";
    TVM_FFI_ICHECK_EQ(output.size(1), shape_n) << "Output N dimension must match weight N";
